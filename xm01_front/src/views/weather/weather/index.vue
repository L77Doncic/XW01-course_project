  ...
  <div class="main">
      ...
      <div id="location">{{ locationText }}</div>
			<div class="main-grid1">
				...
				<h2>温度: <sup class="degree"><h v-if="apiweatherData">{{ apiweatherData.temp }}</h>°</sup><span>C</span></h2>
				<p>{{city}}</p>
...
async function SubmitDataToDB() {
  try {
    ...
    console.log("apiweatherData:", apiweatherData.value);
    console.log("fuweatherData:", fuweatherData.value);

    // 获取当前时间
    const now = new Date().toISOString();
    const obsTime = parseObsTime(apiweatherData.value.obsTime);
    // 检查 fuweatherData 是否为数组并且至少有一个元素
    if (!Array.isArray(fuweatherData.value) || fuweatherData.value.length === 0) {
      throw new Error("fuweatherData is not an array or it is empty");
    }
    ...
    const formData = {
      ...
      obsTime: obsTime,
      currentTemp: ensureNumeric(apiweatherData.value.temp),
      tempMax: ensureNumeric(fuweatherData.value[0]?.tempMax),
      tempMin: ensureNumeric(fuweatherData.value[0]?.tempMin),
      currentWeatherText: apiweatherData.value.text || null,
      day1FxDate: fuweatherData.value[1]?.fxDate
        ? new Date(fuweatherData.value[1].fxDate)
        : null,
      day1TempMax: ensureNumeric(fuweatherData.value[1]?.tempMax),
      day1TempMin: ensureNumeric(fuweatherData.value[1]?.tempMin),
      day2FxDate: fuweatherData.value[2]?.fxDate
        ? new Date(fuweatherData.value[2].fxDate)
        : null,
      day2TempMax: ensureNumeric(fuweatherData.value[2]?.tempMax),
      day2TempMin: ensureNumeric(fuweatherData.value[2]?.tempMin),
      day3FxDate: fuweatherData.value[3]?.fxDate
        ? new Date(fuweatherData.value[3].fxDate)
        : null,
      day3TempMax: ensureNumeric(fuweatherData.value[3]?.tempMax),
      day3TempMin: ensureNumeric(fuweatherData.value[3]?.tempMin),
      day4FxDate: fuweatherData.value[4]?.fxDate
        ? new Date(fuweatherData.value[4].fxDate)
        : null,
      day4TempMax: ensureNumeric(fuweatherData.value[4]?.tempMax),
      day4TempMin: ensureNumeric(fuweatherData.value[4]?.tempMin),
      createdAt: now,
    };

    // 调用 addWeather 函数提交数据
...
function parseObsTime(obsTime) {
  try {
    return obsTime ? new Date(obsTime) : null;
  } catch (error) {
    console.error("Failed to parse obsTime:", obsTime, error);
    return null;
  }
}
</script>
