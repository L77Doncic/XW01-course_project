Page({
    ...
    wx.request({
      ...
      success:function(res){
        let matchedItems = res.data.aaData.filter(item => {
          return item.id === ress || item.title === ress || item.content === ress || item.limittime === ress || item.begintime === ress;
        });
        // 处理匹配到的项
  ...
  getTodoRecord1:function(){
    let that=this;
    wx.request({
      url: 'http://localhost:9999/TeachDemo_war_exploded/device_data_servlet_action?action=get_device_record1',
      data:{},
      header:{"content-type":"application/x-www-form-urlencoded","x-requested-with":"XMLHttpRequest"},
      success:function(res){
        that.handleGetTodoRecordResult(res);
  ...
  getTodoRecord2:function(){
    let that=this;
    wx.request({
      url: 'http://localhost:9999/TeachDemo_war_exploded/device_data_servlet_action?action=get_device_record2',
      data:{},
      header:{"content-type":"application/x-www-form-urlencoded","x-requested-with":"XMLHttpRequest"},
      success:function(res){
        that.handleGetTodoRecordResult(res);
  ...
  handleGetTodoRecordResult:function(res){
    //console.log(JSON.stringify(res.data.aaData));
    this.setData({
  ...
  onDeleteRecord(e){
    ...
    wx.showModal({
      ...
      complete: (res) => {
        ...
        let that=this;
        wx.request({
          url: 'http://localhost:9999/TeachDemo_war_exploded/device_data_servlet_action?action=delete_device_record',
          data:{"id":id},
          header:{"content-type":"application/x-www-form-urlencoded","x-requested-with":"XMLHttpRequest"},
         success:function(res){
       // that.handleGetTodoRecordResult(res);
  ...
  onModifyRecord:function(e){
    ...
    wx.navigateTo({
      url: 'todo_modify?record='+id,
    })
  },
  onAddReocrd:function() {
    console.log("1111");
    wx.navigateTo({
      url: '/pages/todolist/todo_add',
    })
