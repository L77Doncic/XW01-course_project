...
def Press_Yellow_Dectate(scene, carTmpLocation, offenseRecord, i, resultImage, image, saveXM9, conn, cur):
    [x2, y2, w2, h2] = carTmpLocation
    if utils.onLines(scene, [x2, y2, w2, h2]):
        if 3 in offenseRecord[i]:
            cv2.rectangle(resultImage, (x2, y2), (x2 + w2, y2 + h2), rectangleColor_onlines, 10)
        else:
            ...
            # # sql操作
            # sql = "insert into xm01_yellowline values(null,'"+pic_id+"',"+str(x2)+","+str(y2)+","+str(x2+w2)+","+str(y2+h2)+",'"+data_time+"','"+file_path+"',"+str(i)+");"
            # print(sql)
            # cur.execute(sql)
            # conn.commit()
        cv2.rectangle(resultImage, (x2, y2), (x2 + w2, y2 + h2), rectangleColor_onlines, 10)
