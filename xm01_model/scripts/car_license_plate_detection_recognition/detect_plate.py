...
if __name__ == '__main__':
    ...
    if not opt.video:     #处理图片
        ...
        fourcc = cv2.VideoWriter_fourcc(*'MP4V')  #原本MP4v
        ...
        if capture.isOpened():
            while True:
                ...
                cv2.resizeWindow(window_name, desired_width, desired_height)  # 调整窗口的大小
                center_window(window_name, desired_width, desired_height)  # 调用函数将窗口移动到屏幕中心
        ...
        capture.release()
        out.release()
        print("Video output writing complete.")
